        ...
        for ( var prop in mat ) {
            ...
            switch ( prop.toLowerCase() ) {
                ...
                case 'origposition':
                    ...
                    let xyzArray1 = value.split(" ");
                    let origPosition = new THREE.Vector3( parseFloat(xyzArray1[0]), parseFloat(xyzArray1[1]), parseFloat(xyzArray1[2]) );
                    params.userData.origPosition = origPosition;

                    break;

                case 'scale':

                    let xyzArray2 = value.split(" ");
                    let scale = new THREE.Vector3( parseFloat(xyzArray2[0]), parseFloat(xyzArray2[1]), parseFloat(xyzArray2[2]) );
                    params.userData.scale = scale;
	     ...
	     // Update imageInfoVecGlobal
             ...
             for (let i=0; i<imageFilenameArray.length; i++) {
                 ...
                 let suffix = imageFilename.split('.').pop();
                 console.log('suffix', suffix);

                 if(suffix === "jpeg")
                 {
                     // call the setMapForType function that loads the texture
                     // only for jpeg images (as indicator of images of xxx.structure.layerXX.mtl)
                     // (when running with mode "doSkipJpg", the images of overlay.mtl (.jpg) are not read from the .zip file, so
                     // calling this function for overlay.mtl causes an error. This is a workaround to prevent this error)
